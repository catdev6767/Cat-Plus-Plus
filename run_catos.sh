...
echo ""
echo -e "${B}${C}═══════════════════════════════════════════════${X}"
echo -e "${B}${C}  CatOS — Verify + Boot                        ${X}"
echo -e "${B}${C}═══════════════════════════════════════════════${X}"
echo ""
...
if [ ! -f kernel/catos.elf ]; then
    echo -e "  ${R}✗ Chưa có catos.elf${X}"
    echo "  Chạy: bash build_catos.sh"
    exit 1
fi

echo "  ELF info:"
file kernel/catos.elf | sed 's/^/    /'

echo ""
echo "  Size:"
size kernel/catos.elf | sed 's/^/    /'

echo ""
echo "  Entry point:"
readelf -h kernel/catos.elf | grep "Entry" | sed 's/^/    /'

echo ""
echo "  Multiboot header:"
MAGIC=$(xxd -p -l 4 kernel/catos.elf 2>/dev/null)
# Tìm magic 0x1BADB002 trong 8KB đầu
if xxd kernel/catos.elf 2>/dev/null | head -512 | grep -q "02b0ad1b\|1badb002"; then
    echo -e "    ${G}✓ Multiboot magic found${X}"
else
    # Kiểm tra section .multiboot
    if readelf -S kernel/catos.elf | grep -q multiboot; then
...
strings kernel/catos.elf 2>/dev/null | grep -i "catos\|cat++\|hello\|kernel" | head -10 | sed 's/^/    /'
...
if command -v qemu-system-x86_64 >/dev/null 2>&1; then
    ...
    if command -v distrobox >/dev/null 2>&1; then
        ...
        echo ""
        echo -e "  ${Y}→ Sẽ chạy CatOS trong container${X}"
        QEMU_MODE="distrobox"
    else
        echo ""
...
echo "  ────────────────────────────────────────────"
echo ""
echo -e "${G}✓ CatOS đã dừng${X}"
echo ""

# ═══ 4. Nếu muốn xem giao diện đồ họa ═══
echo -e "${B}Muốn xem cửa sổ đồ họa?${X}"
echo "  Chạy:"
echo "    qemu-system-x86_64 -kernel kernel/catos.elf -m 128M"
echo ""
