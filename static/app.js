...
function initSplitEditor() {
    if (editor2) return;
    var el2 = document.getElementById('editor-2');
    if (!el2) return;
    
    editor2 = monaco.editor.create(el2, {
        value: '// Editor 2 — chia đôi màn hình\n// Ctrl+\\ để đóng\n',
        language: 'catpp',
        theme: document.body.classList.contains('cat-theme') ? 'catpp-cat' :
               (localStorage.getItem('catpp:theme') === 'light' ? 'catpp-light' : 'catpp-dark'),
        automaticLayout: true,
        fontSize: currentFontSize,
        fontFamily: 'Consolas, "Ubuntu Mono", monospace',
    ...
    console.log('[Cat++] Editor 2 đã tạo');
}

function toggleSplit() {
    var wrap = document.getElementById('editor-split');
    ...
    if (splitActive) {
        ...
        if (btn) btn.classList.add('split-active');
        // Focus editor 2
        setTimeout(function() { if (editor2) editor2.focus(); }, 100);
    } else {
        wrap.classList.add('single');
        if (btn) btn.classList.remove('split-active');
        setTimeout(function() { if (editor) editor.focus(); }, 100);
    }
    // Trigger Monaco relayout
    if (editor) editor.layout();
    if (editor2) editor2.layout();
    console.log('[Cat++] Split:', splitActive);
}

// Sync theme cho editor 2
var _origApplyTheme = null;
if (typeof applyTheme === 'function') {
    _origApplyTheme = applyTheme;
...
// Bind events sau khi boot
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        var btn = document.getElementById('split-btn');
        if (btn) btn.onclick = toggleSplit;
        ...
        if (divider) {
            ...
            document.addEventListener('mousemove', function(e) {
                ...
                var newLeft = Math.max(200, Math.min(wrapW - 200, startLeft + dx));
                var pct = (newLeft / wrapW * 100).toFixed(1);
                wrap.style.gridTemplateColumns = pct + '% 1px 1fr';
                if (editor) editor.layout();
                if (editor2) editor2.layout();
            });
