...
static Value fact(Value n) {
    if (v_truthy(V_bool(v_cmp(n, V_int(1LL), 4)))) {
        return V_int(1LL);
    }
    return v_mul(n, fact(v_sub(n, V_int(1LL))));
    return V_null();
}
...
int main(void) {
    ...
    v_print(v_sub(x, y));
    v_print(v_mod(x, y));
    v_print(v_div(V_int(10LL), V_int(2LL)));
    v_print(v_div(V_int(10LL), V_int(4LL)));
    v_print(add(V_int(5LL), V_int(7LL)));
    v_print(fact(V_int(5LL)));
    i = V_int(1LL);
    while (v_truthy(V_bool(v_cmp(i, V_int(5LL), 4)))) {
        v_print(i);
        i = v_add(i, V_int(1LL));
    }
    name = V_str("Meo");
    v_print(v_add(v_add(V_str("Xin chao, "), name), V_str("!")));
    return 0;
}