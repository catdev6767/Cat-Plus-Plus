...
static Value __meth_0_0(Value me, Value* __args, int __argc);
static Value __meth_0_1(Value me, Value* __args, int __argc);
static Value __meth_0_2(Value me, Value* __args, int __argc);

static Value __meth_0_0(Value me, Value* __args, int __argc) {
    Value a = __argc > 0 ? __args[0] : V_null();
    Value b = __argc > 1 ? __args[1] : V_null();
    v_dot_set(me, "x", a);
    v_dot_set(me, "y", b);
    return V_null();
}
...
static Value __meth_0_1(Value me, Value* __args, int __argc) {
    return v_add(v_add(v_add(v_add(V_str("("), bi_say(v_dot(me, "x"))), V_str(", ")), bi_say(v_dot(me, "y"))), V_str(")"));
    return V_null();
}

static Value __meth_0_2(Value me, Value* __args, int __argc) {
    Value dx = __argc > 0 ? __args[0] : V_null();
    Value dy = __argc > 1 ? __args[1] : V_null();
    v_dot_set(me, "x", v_add(v_dot(me, "x"), dx));
    v_dot_set(me, "y", v_add(v_dot(me, "y"), dy));
    return V_null();
}

static MethodEntry __methods_0[] = {
    { "new", __meth_0_0 },
    { "info", __meth_0_1 },
    { "move", __meth_0_2 },
};
static char* __fields_0[] = {
    "x",
    "y",
};
static ClassDef __clsd_0 = { "Point", NULL, __methods_0, 3, __fields_0, 2 };

static Value __meth_1_0(Value me, Value* __args, int __argc);

static Value __meth_1_0(Value me, Value* __args, int __argc) {
    Value n = __argc > 0 ? __args[0] : V_null();
    v_dot_set(me, "name", n);
    return V_null();
}

static MethodEntry __methods_1[] = {
    { "new", __meth_1_0 },
};
static char* __fields_1[] = {
    "name",
};
static ClassDef __clsd_1 = { "Animal", NULL, __methods_1, 1, __fields_1, 1 };

static Value __meth_2_0(Value me, Value* __args, int __argc);
static Value __meth_2_1(Value me, Value* __args, int __argc);
...
static Value __meth_2_1(Value me, Value* __args, int __argc) {
    return v_add(v_dot(me, "name"), V_str(" says woof"));
    return V_null();
}

static MethodEntry __methods_2[] = {
    { "new", __meth_2_0 },
    { "speak", __meth_2_1 },
};
static char* __fields_2[] = {
    NULL,
};
static ClassDef __clsd_2 = { "Dog", "Animal", __methods_2, 2, __fields_2, 0 };
...
int main(void) {
    ...
    Value x;
    Value z;
    p = v_new("Point", (Value[]){V_int(3LL), V_int(4LL)}, 2);
    v_print(v_call_method(p, "info", NULL, 0));
    (void)v_call_method(p, "move", (Value[]){V_int(10LL), V_int(20LL)}, 2);
    v_print(v_call_method(p, "info", NULL, 0));
    d = v_new("Dog", (Value[]){V_str("Rex")}, 1);
    v_print(v_call_method(d, "speak", NULL, 0));
    x = V_int(2LL);
    { Value __t1 = x;
        if (v_cmp(__t1, V_int(1LL), 0)) {
            v_print(V_str("one"));
        }
        else if (v_cmp(__t1, V_int(2LL), 0)) {
            v_print(V_str("two"));
        }
        else {
            v_print(V_str("other"));
    ...
    { jmp_buf __jb___t2; int __save___t2 = __jmp_top; __jmp_stack[__jmp_top++] = &__jb___t2;
      if (setjmp(__jb___t2) == 0) {
            z = v_div(V_int(1LL), V_int(0LL));
      } else {
            Value e = V_str(__catpp_err_msg);
            v_print(V_str("bat duoc loi"));
      }
      __jmp_top = __save___t2;
    }
    v_print(V_int(v_cast_int(V_str("42"))));
    v_print(V_int(8));
    return 0;
}