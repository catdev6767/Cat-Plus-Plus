...
int main(void) {
    Value p;
    p = v_struct_new("P", (const char*[]){"x", "y"}, 2, (Value[]){V_int(3LL), V_int(4LL)}, 2);
    v_print(v_add(v_dot(p, "x"), v_dot(p, "y")));
    v_print(V_int(v_sizeof_type("i8")));
    v_print(V_int(v_sizeof_type("i32")));
    v_print(V_int(v_sizeof_type("i64")));
    v_print(sq(V_int(5LL)));
    return 0;
}